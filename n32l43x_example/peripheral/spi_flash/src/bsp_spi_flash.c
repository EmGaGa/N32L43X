...
uint8_t SPI_FLASH_SendByte(uint8_t byte)
{
	...
	/* 等待发送缓冲区为空，TXE事件 */
	while (SPI_I2S_GetStatus(SPI_FLASH_PERIPH, SPI_I2S_TE_FLAG) == RESET)
	{
		if((SPITimeout--) == 0) return SPI_TIMEOUT_UserCallback(0);
	}

	/* 写入数据寄存器，把要写入的数据写入发送缓冲区 */
	SPI_I2S_TransmitData(SPI_FLASH_PERIPH, byte);

	SPITimeout = SPIT_FLAG_TIMEOUT;
	/* 等待接收缓冲区非空，RXNE事件 */
	while (SPI_I2S_GetStatus(SPI_FLASH_PERIPH, SPI_I2S_RNE_FLAG) == RESET)
	{
		if((SPITimeout--) == 0) return SPI_TIMEOUT_UserCallback(1);
	}

	/* 读取数据寄存器，获取接收缓冲区数据 */
	return SPI_I2S_ReceiveData(SPI_FLASH_PERIPH);
}
...
uint16_t SPI_FLASH_SendHalfWord(uint16_t HalfWord)
{
	...
	/* 写入数据寄存器，把要写入的数据写入发送缓冲区 */
	SPI_I2S_TransmitData(SPI_FLASH_PERIPH, HalfWord);
